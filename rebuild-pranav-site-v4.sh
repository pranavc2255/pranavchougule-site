...
echo "==> Rebuilding (Next 16 + Tailwind v4) in: $ROOT"
echo "==> Project: $PROJECT"

# 0) Kill port 3000 (prevents "site not loading")
...
# 1) Remove old project
rm -rf "${PROJECT}"

# 2) Create fresh Next.js app (with Tailwind)
npx create-next-app@latest "${PROJECT}" \
  --ts --eslint --tailwind --app --src-dir --import-alias "@/*" --use-npm --yes

cd "${PROJECT}"

# 3) Ensure Tailwind v4 PostCSS plugin is installed
npm install -D @tailwindcss/postcss autoprefixer

# 4) Tailwind v4 compatible PostCSS config
...
# 5) globals.css (Tailwind directives still valid)
            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active ? "bg-blue text-white shadow-soft" : "text-blue hover:bg-white hover:text-blue2",
                ].join(" ")}
              >
                {t.label}
              </Link>
              ...
              return (
                <Link
                  ...
                  className={[
                    ...
                    active ? "bg-blue text-white border-blue" : "bg-white text-blue border-border",
                  ].join(" ")}
                >
                  {t.label}
                </Link>
...
export default function Research() {
  return (
    <PageShell title="Research" subtitle="Robotics and AI for Infrastructure NDE" accent="blue">
      <div className="max-w-3xl space-y-6 text-muted leading-relaxed">
        <p>
          ...
          I focus on synchronized workflows where robot motion, sensing, and data logging are designed as a single system.
        </p>
        <ul className="list-disc pl-5 space-y-2">
          <li>Multimodal sensing: UPV, AE, hyperspectral imaging, laser-based surface monitoring</li>
          <li>AI interpretation: Florence-based VLM workflows for UPV signal representations</li>
    ...
    <PageShell title="Systems" subtitle="Multimodal sensing + AI interpretation + reconstruction" accent="saffron">
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <Box title="Florence VLM for UPV">
          Vision-language reasoning over ultrasonic waveform representations for context-aware NDE analytics.
        </Box>
  ...
  return (
    <PageShell title="Publications" subtitle="Accepted / in-progress" accent="saffron">
      <div className="max-w-4xl">
        <div className="rounded-xl2 border border-border bg-soft p-6">
          <div className="text-ink font-semibold">Chougule, P., Farnam, Y., Ebrahimkhanlou, A.</div>
